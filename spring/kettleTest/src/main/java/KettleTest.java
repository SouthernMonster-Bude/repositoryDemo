...
/**
 * @author Houjiemu
 * @create 2020-11-05 11:56
 */
public class KettleTest {
    //存放读取到的xml字符串
    private static String[] databasesXML;
    //kettle插件的位置
    private static final String KETTLE_PLUGIN_BASE_FOLDER = "D:\\DevelopTools\\dataTools\\data-integration\\plugins";
    ...
    public static void main(String[] args) throws KettleException, IOException {
        ...
        String rootPath = KettleTest.class.getResource("/").getPath();
        ...
        for (File xml : Objects.requireNonNull(dbDir.listFiles())) {
            if (xml.isFile() && xml.getName().endsWith(".xml")) {
                xmlStrings.add(FileUtils.readFileToString(xml));
            }
        }
...
//        );
        ...
        String fileName = rootPath+"/test.ktr";
